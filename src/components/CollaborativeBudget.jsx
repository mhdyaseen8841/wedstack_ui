...
export default function CollaborativeBudget({ wedding, vendors, expenses = [], token, side, user, onUpdateWedding }) {
  ...
  const [brideTargetInput, setBrideTargetInput] = useState(wedding.brideBudget || 0);
  const [filterSide, setFilterSide] = useState('All');
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState(null);

  useEffect(() => {
    setGroomTargetInput(wedding.groomBudget || 0);
    setBrideTargetInput(wedding.brideBudget || 0);
  }, [wedding]);
  ...
  const getExpensesFromLedger = (paidBySide) => {
    // Parse expenses and match by paidBy side
    return expenses
      .filter(e => e.paidBy === paidBySide)
      .reduce((sum, e) => sum + (e.amount || 0), 0);
  };

  const groomSpentOnly = getExpenses('Groom') + getExpensesFromLedger('Groom');
  const brideSpentOnly = getExpenses('Bride') + getExpensesFromLedger('Bride');
  const sharedSpentTotal = getExpenses('Shared') + getExpensesFromLedger('Shared');
  ...
  const handleSaveBudget = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:5000/api/wedding', {
        ...
        headers,
        body: JSON.stringify({
          groomBudget: Number(groomTargetInput) || 0,
          brideBudget: Number(brideTargetInput) || 0
        })
      });
    ...
    <div className="space-y-6 text-slate-800">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
        <div className="flex justify-between items-center border-b border-slate-100 pb-3 flex-wrap gap-2">
          ...
          {message && (
            <span className={`text-xs px-2.5 py-1 rounded-full font-bold ${message.type === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'
              }`}>
              {message.text}
            </span>
          )}
